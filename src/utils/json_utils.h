...
LIBMINI_API JsonValue parse_json(const std::string& json_str);

// 序列化为 JSON 文本（紧凑格式）
LIBMINI_API std::string to_json_string(const JsonValue& value);

// 解析 JSON 文本为紧凑格式字符串。
//   {"name":"test","value":123}  →  {"name":"test","value":123}
//   { "a" : 1 }                  →  {"a":1}
// 非法输入返回空串（与 parse_json 不同，不抛异常）。
LIBMINI_API std::string parse_json_simple(const std::string& json_str);
...
LIBMINI_API std::string to_json_string_simple(const std::string& value);

// 转义字符串内容的 JSON 转义文本（不含首尾引号）。
// 用于手工拼接 JSON 时转义动态值：
//   "{\"msg\":\"" + escape_json_string(user_input) + "\"}"
LIBMINI_API std::string escape_json_string(const std::string& value);
